    ...
    public void run() {
        synchronized (mutex) {
            try {
                ...
                ObjectNode rootNode = (ObjectNode) mapper.readTree(new File("C:\\Users\\ahmed\\IdeaProjects\\devoirThreqds\\Folders\\input.json"));
                ...
                mapper.writerWithDefaultPrettyPrinter().writeValue(new File("C:\\Users\\ahmed\\IdeaProjects\\devoirThreqds\\Folders\\output.json"), outputRoot);

                ObjectNode errorRoot = mapper.createObjectNode();
                errorRoot.set("orders", mapper.valueToTree(invalidOrders));
                mapper.writerWithDefaultPrettyPrinter().writeValue(new File("C:\\Users\\ahmed\\IdeaProjects\\devoirThreqds\\Folders\\error.json"), errorRoot);
